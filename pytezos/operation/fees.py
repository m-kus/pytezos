...
def default_fee(content):
    return calculate_fee(
        ...
        + 64
        + 3
        * 3,  # branch, signature, fee:gas_limit:storage_limit mutez values (+3 bytes)
    )
...
def default_gas_limit(content):
    values = {
        ...
        "transaction": hard_gas_limit_per_operation
...
def default_storage_limit(content):
    values = {
        ...
        "transaction": hard_storage_limit_per_operation
        if content.get("parameters")
        else 257,
    }
    return values.get(content["kind"])
