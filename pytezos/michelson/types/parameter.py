...
class ParameterType(MichelsonType, prim='parameter', args_len=1):
    ...
    def list_entry_points(cls) -> Dict[str, Type[MichelsonType]]:
        ...
        if cls.args[0].prim == 'or':
            flat_args = Struct.get_flat_args(cls.args[0], force_recurse=True, fields_only=True)
    ...
    @classmethod
    def from_parameters(cls, parameters: Dict[str, Any]) -> 'ParameterType':
        if not parameters:
        ...
        if cls.args[0].prim == 'or':
            struct = Struct.from_nested_type(cls.args[0], force_recurse=True)
    ...
    @classmethod
    def from_micheline_value(cls, val_expr) -> 'ParameterType':
        item = cls.args[0].from_micheline_value(val_expr)
        return cls(item)

    @classmethod
    def from_python_object(cls, py_obj) -> 'ParameterType':
    ...
    def merge_lazy_diff(self, lazy_diff: List[dict]) -> 'ParameterType':
