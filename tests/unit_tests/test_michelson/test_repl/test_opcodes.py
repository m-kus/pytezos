...
class OpcodesTestCase(TestCase):
    ...
    @parameterized.expand(
        [
            ...
            # GROTH16
            ('groth16.tz', 'Unit', f'Pair (Pair {input_x} {input_y}) (Pair (Pair {proof_a} {proof_b}) {proof_c})', 'Unit'),
        ]
    )
    ...
    @parameterized.expand(
        [
            ...
            # Test signature
            ('check_signature.tz', f'(Pair "{SIGNATURE}" "abcd")', '"edpkuBknW28nW72KG6RoHtYW7p12T6GKc7nAbwYX5m8Wd9sDVC9yav"'),
            # BLS12_381_Fr overflow
            ('bls12_381_fr_to_int.tz', '0', '0xf7ef66f95c90b2f953eb0555af65f22095d4f54b40ea8c6dcc2014740e8662c16bb8786723'),
        ]
    )
    def test_failed_opcodes(self, filename, storage, parameter):
        ...
        _, _, _, _, error = Interpreter.run_code(
            parameter=michelson_to_micheline(parameter), storage=michelson_to_micheline(storage), script=michelson_to_micheline(script)
        )
