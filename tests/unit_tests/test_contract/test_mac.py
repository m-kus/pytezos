...
from pytezos import ContractInterface, Unit, pytezos
    ...
    'assets': {
        'hook': {
            ...
            'permissions_descriptor': {
                'custom': {'config_api': None, 'tag': 'none'},
...
class TestMac(TestCase):
    ...
    def test_pause(self):
        res = self.mac.pause(True).interpret(
            storage=initial_storage, source=pytezos.key.public_key_hash(), sender=pytezos.key.public_key_hash()
        )
    ...
    def test_is_operator_callback(self):
        res = self.mac.is_operator(
            ...
            operator={'operator': pytezos.key.public_key_hash(), 'owner': pytezos.key.public_key_hash(), 'tokens': {'all_tokens': Unit}},
    ...
    def test_transfer(self):
        ...
        res = self.mac.transfer([dict(amount=1000, from_=pkh, to_='tz1abavrqYNzpcDPiQURyCt1THti8J27W6mR', token_id=0)]).interpret(
            storage=initial_storage_balance, source=pkh
        )
        self.assertEqual(len(res.lazy_diff), len(set(map(lambda x: x['id'], res.lazy_diff))))
        self.assertDictEqual({(pkh, 0): 41000, ('tz1abavrqYNzpcDPiQURyCt1THti8J27W6mR', 0): 1000}, res.storage['assets']['ledger'])
